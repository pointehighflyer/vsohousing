...
class Host{
	...
	function buildList($r, $id){
		...
		while($data){
			...
			$list .= "\t".'<li class="record';
			
			if($id == $data['pkHostID']) $list .=' active';
			
			$list .='"><a href="'.$_SERVER['PHP_SELF'].'?id='.$data['pkHostID'].'#'.$data['pkHostID'].'" id="'.$data['pkHostID'].'">'.
	...
	function getNameArray($r){
		...
		while($data){
			...
			$arr[$data['fldLastName'].', '.$data['fldFirstName']] = $data['pkHostID'];
